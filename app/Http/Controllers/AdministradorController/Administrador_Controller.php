...
namespace MickeyMouse2017\Http\Controllers\AdministradorController;
...
use MickeyMouse2017\Models\Equipos\Tipo_Equipo;
...
class Administrador_Controller extends Controller{
	...
	public function Listar_Usuarios(){
		...
		foreach ($Empleados  as $resultados) {		
			$Empleadoss[$resultados->id] = strtoupper($resultados->nombre_usuario.' '.$resultados->apellido.'---Codigo: '.$resultados->codigo);
		}		
		return $Empleadoss;
	}
	public function Listar_Turnos(){
		...
		foreach ($Turnos  as $resultados) {	
			...
			$Turnoss[$resultados->id] = strtoupper($resultados->nombre_turno.':'.' '.$HoraTurnoInicio.'---'.$HoraTurnoFin);
		}		
		return $Turnoss;
	}
	...
	public function Listar_Formularios(){
		...
		foreach ($Formularios  as $resultados) {				
			...
			$Formularioss[$resultados->id] = strtoupper($resultados->nombre_formulario);
		}		
		return $Formularioss;
	}
	...
	public function ConsultarAsignacionUsuario(){
		$NombreUsuarioBuscar=Input::get('NombreUsuarioBuscar');	
		$Asignaciones=Asignaciones_M::Where('fk_usuarios',$NombreUsuarioBuscar)->paginate(10);
	...
	public function Listar_Nombres_Formularios(){
		...
		foreach ($NombreFormularios  as $resultados) {			
			...
			$NombresFormularios[$resultados->id] = strtoupper($resultados->nombre_formulario."   (".$resultados->Nombre_Equipo->nombre_equipo.")");
		}		
...
//-----------------------------------equipos-----------------------------------------------
	...
	public function Listar_Tipos_Equipos_C(){
		...
		foreach ($DatosTipoEquipos  as $resultados) {			
			...
			$TipoEquipo[$resultados->id] = strtoupper($resultados->nombre_tipo_equipos);
		}		
		return $TipoEquipo;
	}
...
public function Autocompletar_consolidado_C(){ //--------------------------------
	...
	foreach ($Asignaciones as $key => $value) {
		...
		$NombreUsuario=strtoupper($value->Nombre_Usuario->nombre_usuario)." ". strtoupper($value->Nombre_Usuario->apellido);
		$NombreFormulario=strtoupper($value->Nombre_Formulario->nombre_formulario);
		$NombreTurno=strtoupper($value->Nombre_Turno->nombre_turno);	
	...
	public function Listar_datos_turnos_C (){
		...
		if($DAtosTurnos!='[]'){
			foreach ($DAtosTurnos  as $resultados) {
				...
				$DatosTurno[$resultados->fk_turno] =strtoupper($resultados->Nombre_Turno->nombre_turno.':'.' '.$HoraTurnoInicio.'--'.$HoraTurnoFin);
	...
	public function Listar_Formularios_C (){
		...
		if($DAtosTurnos!='[]'){
			foreach ($DAtosTurnos  as $resultados) {				
				...
				$DatosTurno[$resultados->id] =strtoupper($resultados->Nombre_Formulario->nombre_formulario.' - '.'codigo '.$resultados->Nombre_Formulario->id);
...
// Aqui empieza todo de parametros
	...
	public function Listar_Parametros_Vista_Parametros(){
		...
		foreach ($Nombres_Parametros  as $resultados) {	
			$nombre_parametro=strtoupper($resultados->nombre_parametro);
	...
	public function Listar_Unidad_Vista_Parametros(){
		...
		if($Nombres_Unidades!="[]"){
			...
			foreach ($Nombres_Unidades  as $resultados) {
				$nombre_unidad=strtoupper($resultados->nombre_unidad);
	...
	public function CargarVistaTurnos(){
		return view('Administrador/Formularios.Create_Turnos');
	}

	public function Cargar_Tabla_Turnos(){
		$Listar_Turnos=Turnos_M::paginate(20);
		
		return view('Administrador/Tablas.Tabla_Turnos')
		->with('Listar_Turnos',$Listar_Turnos);
	}

	public function Registrar_Nuevo_Turno(){
