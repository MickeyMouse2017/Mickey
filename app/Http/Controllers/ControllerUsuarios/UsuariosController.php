...
namespace MickeyMouse2017\Http\Controllers\ControllerUsuarios;


use Illuminate\Http\Request;
use MickeyMouse2017\Http\Controllers\Controller;
use MickeyMouse2017\Models\Usuarios\Usuario;
use MickeyMouse2017\Models\Usuarios\Conexion_Usuarios;
...
class UsuariosController extends Controller {
	...
	public function Logueo(){	
		...
		if($EstadoUsuario==""){
			return Response::json(['ErrorInactivo' =>false,
				'errors'=>'La cuenta ingresada no se encuentra registrada en el sistema.']);
		}else{			
	...
	public function Listar_Roles(){
		...
		foreach ($Rol  as $resultados) {		
			$Roles[$resultados->id] = strtoupper($resultados->nombre_rol);
		}		
		return $Roles;
	}
	...
	public function Listar_Nombres_Usuarios(){
		...
		foreach ($Usuarios  as $resultados) {		
			$Usuario[$resultados->id] = strtoupper($resultados->nombre_usuario).' '.strtoupper($resultados->apellido).' '.'--------- CODIGO:'.$resultados->codigo;
		}		
		return $Usuario;
	}
	...
	public function Cargar_Datos_Perfil_Usuario(){
		...
		foreach ($Usuario as $key => $value) {
			$NombrePerfil=strtoupper($value->nombre_usuario).' '.strtoupper($value->apellido);
			$RangoPerfil=strtoupper($value->Nombre_Rol->nombre_rol);
