<?php
namespace MickeyMouse2017\Http\Controllers\Controller_Formulario;
use Illuminate\Http\Request;
use MickeyMouse2017\Http\Requests;
use MickeyMouse2017\Http\Controllers\Controller;
use MickeyMouse2017\Models\Formulario\Encabezado_Formulario;
use MickeyMouse2017\Models\Formulario\Detalle_Formulario;
use MickeyMouse2017\Models\Formulario\Consolidado_Formularios;
use MickeyMouse2017\Models\Equipos\Equipo;
use MickeyMouse2017\Models\Asignaciones\Asignaciones_M;
...
class FormularioController extends Controller{
	...
	public function Consultar_Turno(){
		...
		$Asignaciones=Asignaciones_M::Where('fk_usuarios',$id_Usuario_Logueado)
		->Where('estado',"Activo")		
		->get();
		...
		foreach ($Asignaciones as $key => $value) {
			...
			$Id_Formulario_Seleccionado=$value->fk_formulario;
			
			// if($fecha_fin<=$Fecha_Actual){
			// 	// Deshabilito la asignacion
			// 	dd("Se vencio el formulario");
			// }
			
			
				// dd($hora_inicio,$hora_fin,$horaActual);


			if($fecha_fin <= $Fecha_Actual){

				$Turnos=Turnos_M::Where('id',$Turno)				
				->get();

				foreach ($Turnos as $key => $value) {
					$hora_inicio=$value->hora_inicio;
					$hora_fin=$value->hora_fin;
					$turno_actual=$value->id;

					if($horaActual>=$hora_inicio && $horaActual<=$hora_fin){
									// Pintaria informacion
						$Registro_En_turno=Consolidado_Formularios::where('fk_usuario',$id_Usuario_Logueado)
						->where('fecha_ingreso',$Fecha_Actual)
						->Where('fk_encabezado_formulario',$Id_Formulario_Seleccionado)
						->where('fk_turno',$turno_actual)->get();

						if($Registro_En_turno=="[]"){
							$UltimaVersionForm=Detalle_Formulario::Where('fk_formulario',$Id_Formulario_Seleccionado)
							...
							$Detalle_Formulario=Detalle_Formulario::Where('fk_formulario',$Id_Formulario_Seleccionado)
							...
							->with('turno_actual',$turno_actual)
							...
							$Id_proximo =Turnos_M::Where('id','>',$turno_actual)				
							->get();
							foreach ($Id_proximo as $key => $value) {
								$hora_inicio=$value->hora_inicio;

								if($horaActual>="01:00:01" && $horaActual<="23:59:59"){
									$variable=Carbon::today()->toFormattedDateString()." ".$hora_inicio;
									return view('Empleados/Tablas.Tabla_Temporizador_Ruta')
									->with('variable',$variable);
								}else{
									$variable=Carbon::tomorrow()->toFormattedDateString()." ".$hora_inicio;
									return view('Empleados/Tablas.Tabla_Temporizador_Ruta')
									->with('variable',$variable);
								}
							}
						...
						foreach ($ultimo as $key => $value) {
							$Id_Turno_Actual=$value->id;
						}
						// $Id_Turno_Actual=$id;

						if($Id_Turno_Actual==$turno_actual){

							$turno_actual_usuario=DB::table('turnos')->first();	
							$Id_Turno_Actual=$turno_actual_usuario->id;					
						}

						$Id_proximo =Turnos_M::Where('id','>',$Id_Turno_Actual)				
						->get();

						// dd($Id_proximo,$turno_actual);
						foreach ($Id_proximo as $key => $value) {
							$hora_inicio=$value->hora_inicio;
						...
						// dd($hora_inicio,$Id_proximo,$horaActual);
					}
				}
	...
	function Consultar_Rutas_Empleado(){
		...
		$Cantidad_Asignaciones_En_turno=Asignaciones_M::Where('fk_usuarios',$id_Usuario_Logueado)
		->Where('estado',"Activo")
		->Where('fk_turno',$turno)
		->count();
		...
		// }
		// dd($Asignaciones_En_turno);



		$MensajeVista="Tienes ".$Cantidad_Asignaciones_En_turno." Rutas asignadas en este turno:";		

		$DatosTurno[0] = "No se encontró ninguna ruta disponible en este horario.";		
		if($Asignaciones_En_turno!='[]'){
			$DatosTurno=[];
			foreach ($Asignaciones_En_turno  as $resultados) {
				$DatosTurno[$resultados->fk_formulario] =strtoupper($resultados->Nombre_Formulario->nombre_formulario);
				// var_dump($DatosTurno);
			}	

			return Response::json(['DatosTurno'=>$DatosTurno,
	...
	public function Cargar_Formulario(){
		...
		$Id_Formulario_Seleccionado=Input::get('id_formulario_turno');
		...
		Where('estado',"Activo")
		->Where('fk_formulario',$Id_Formulario_Seleccionado)
		->get();
...
// TERMINA PRIMER TURNO
		...
		// TERMINA TERCER TURNO
			...
			if($Registro_En_turno=="[]"){			
				...
				// dd($Detalle_Formulario,$id_Formulario_En_turno,$id_version_form_Vigente);
		...
		if(!empty($consolidadoValues['variable_1'])){			
			...
			if($consolidadoValues['variable_1'] != $Valor_Ideal[1]){
				...
				if($consolidadoValues['variable_1'] <$Valor_Minimo[1]){
					$Dato='Valor Minimo';
					$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de:".$consolidadoValues['variable_1'].", Ya que el valor mínimo permitido es de: ".$Valor_Minimo[1].".";
				}else{
					if($consolidadoValues['variable_1'] < $Valor_Ideal[1] && $consolidadoValues['variable_1'] >=$Valor_Minimo[1]){				
						...
						if($consolidadoValues['variable_1'] > $Valor_Ideal[1] && $consolidadoValues['variable_1'] <=$Valor_Maximo[1]){
							...
							if($consolidadoValues['variable_1'] >$Valor_Maximo[1]){				
								$Dato='Valor Maximo';
								$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de: ".$consolidadoValues['variable_1'].", Ya que el valor máximo permitido es de: ".$Valor_Maximo[1].".";
							}
		...
		if(!empty($consolidadoValues['variable_2'])){			
			...
			if($consolidadoValues['variable_2'] != $Valor_Ideal[2]){
				...
				if($consolidadoValues['variable_2'] <$Valor_Minimo[2]){
					$Dato='Valor Minimo';
					$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de:".$consolidadoValues['variable_2'].", Ya que el valor mínimo permitido es de: ".$Valor_Minimo[2].".";
				}else{
					if($consolidadoValues['variable_2'] < $Valor_Ideal[2] && $consolidadoValues['variable_2'] >=$Valor_Minimo[2]){				
						...
						if($consolidadoValues['variable_2'] > $Valor_Ideal[2] && $consolidadoValues['variable_2'] <=$Valor_Maximo[2]){
							...
							if($consolidadoValues['variable_2'] >$Valor_Maximo[2]){				
								$Dato='Valor Maximo';
								$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de: ".$consolidadoValues['variable_2'].", Ya que el valor máximo permitido es de: ".$Valor_Maximo[2].".";
							}
		...
		if(!empty($consolidadoValues['variable_3'])){			
			...
			if($consolidadoValues['variable_3'] != $Valor_Ideal[3]){
				...
				if($consolidadoValues['variable_3'] <$Valor_Minimo[3]){
					$Dato='Valor Minimo';
					$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de:".$consolidadoValues['variable_3'].", Ya que el valor mínimo permitido es de: ".$Valor_Minimo[3].".";
				}else{
					if($consolidadoValues['variable_3'] < $Valor_Ideal[3] && $consolidadoValues['variable_3'] >=$Valor_Minimo[3]){				
						...
						if($consolidadoValues['variable_3'] > $Valor_Ideal[3] && $consolidadoValues['variable_3'] <=$Valor_Maximo[3]){
							...
							if($consolidadoValues['variable_3'] >$Valor_Maximo[3]){				
								$Dato='Valor Maximo';
								$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de: ".$consolidadoValues['variable_3'].", Ya que el valor máximo permitido es de: ".$Valor_Maximo[3].".";
							}
		...
		if(!empty($consolidadoValues['variable_4'])){			
			...
			if($consolidadoValues['variable_4'] != $Valor_Ideal[4]){
				...
				if($consolidadoValues['variable_4'] <$Valor_Minimo[4]){
					$Dato='Valor Minimo';
					$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de:".$consolidadoValues['variable_4'].", Ya que el valor mínimo permitido es de: ".$Valor_Minimo[4].".";
				}else{
					if($consolidadoValues['variable_4'] < $Valor_Ideal[4] && $consolidadoValues['variable_4'] >=$Valor_Minimo[4]){				
						...
						if($consolidadoValues['variable_4'] > $Valor_Ideal[4] && $consolidadoValues['variable_4'] <=$Valor_Maximo[4]){
							...
							if($consolidadoValues['variable_4'] >$Valor_Maximo[4]){				
								$Dato='Valor Maximo';
								$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de: ".$consolidadoValues['variable_4'].", Ya que el valor máximo permitido es de: ".$Valor_Maximo[4].".";
							}
		...
		if(!empty($consolidadoValues['variable_5'])){			
			...
			if($consolidadoValues['variable_5'] != $Valor_Ideal[5]){
				...
				if($consolidadoValues['variable_5'] <$Valor_Minimo[5]){
					$Dato='Valor Minimo';
					$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de:".$consolidadoValues['variable_5'].", Ya que el valor mínimo permitido es de: ".$Valor_Minimo[5].".";
				}else{
					if($consolidadoValues['variable_5'] < $Valor_Ideal[5] && $consolidadoValues['variable_5'] >=$Valor_Minimo[5]){				
						...
						if($consolidadoValues['variable_5'] > $Valor_Ideal[5] && $consolidadoValues['variable_5'] <=$Valor_Maximo[5]){
							...
							if($consolidadoValues['variable_5'] >$Valor_Maximo[5]){				
								$Dato='Valor Maximo';
								$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de: ".$consolidadoValues['variable_5'].", Ya que el valor máximo permitido es de: ".$Valor_Maximo[5].".";
							}
		...
		if(!empty($consolidadoValues['variable_6'])){			
			...
			if($consolidadoValues['variable_6'] != $Valor_Ideal[6]){
				...
				if($consolidadoValues['variable_6'] <$Valor_Minimo[6]){
					$Dato='Valor Minimo';
					$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de:".$consolidadoValues['variable_6'].", Ya que el valor mínimo permitido es de: ".$Valor_Minimo[6].".";
				}else{
					if($consolidadoValues['variable_6'] < $Valor_Ideal[6] && $consolidadoValues['variable_6'] >=$Valor_Minimo[6]){				
						...
						if($consolidadoValues['variable_6'] > $Valor_Ideal[6] && $consolidadoValues['variable_6'] <=$Valor_Maximo[6]){
							...
							if($consolidadoValues['variable_6'] >$Valor_Maximo[6]){				
								$Dato='Valor Maximo';
								$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de: ".$consolidadoValues['variable_6'].", Ya que el valor máximo permitido es de: ".$Valor_Maximo[6].".";
							}
		...
		if(!empty($consolidadoValues['variable_7'])){			
			...
			if($consolidadoValues['variable_7'] != $Valor_Ideal[7]){
				...
				if($consolidadoValues['variable_7'] <$Valor_Minimo[7]){
					$Dato='Valor Minimo';
					$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de:".$consolidadoValues['variable_7'].", Ya que el valor mínimo permitido es de: ".$Valor_Minimo[7].".";
				}else{
					if($consolidadoValues['variable_7'] < $Valor_Ideal[7] && $consolidadoValues['variable_7'] >=$Valor_Minimo[7]){				
						...
						if($consolidadoValues['variable_7'] > $Valor_Ideal[7] && $consolidadoValues['variable_7'] <=$Valor_Maximo[7]){
							...
							if($consolidadoValues['variable_7'] >$Valor_Maximo[7]){				
								$Dato='Valor Maximo';
								$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de: ".$consolidadoValues['variable_7'].", Ya que el valor máximo permitido es de: ".$Valor_Maximo[7].".";
							}
		...
		if(!empty($consolidadoValues['variable_8'])){			
			...
			if($consolidadoValues['variable_8'] != $Valor_Ideal[8]){
				...
				if($consolidadoValues['variable_8'] <$Valor_Minimo[8]){
					$Dato='Valor Minimo';
					$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de:".$consolidadoValues['variable_8'].", Ya que el valor mínimo permitido es de: ".$Valor_Minimo[8].".";
				}else{
					if($consolidadoValues['variable_8'] < $Valor_Ideal[8] && $consolidadoValues['variable_8'] >=$Valor_Minimo[8]){				
						...
						if($consolidadoValues['variable_8'] > $Valor_Ideal[8] && $consolidadoValues['variable_8'] <=$Valor_Maximo[8]){
							...
							if($consolidadoValues['variable_8'] >$Valor_Maximo[8]){				
								$Dato='Valor Maximo';
								$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de: ".$consolidadoValues['variable_8'].", Ya que el valor máximo permitido es de: ".$Valor_Maximo[8].".";
							}
		...
		if(!empty($consolidadoValues['variable_9'])){			
			...
			if($consolidadoValues['variable_9'] != $Valor_Ideal[9]){
				...
				if($consolidadoValues['variable_9'] <$Valor_Minimo[9]){
					$Dato='Valor Minimo';
					$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de:".$consolidadoValues['variable_9'].", Ya que el valor mínimo permitido es de: ".$Valor_Minimo[9].".";
				}else{
					if($consolidadoValues['variable_9'] < $Valor_Ideal[9] && $consolidadoValues['variable_9'] >=$Valor_Minimo[9]){				
						...
						if($consolidadoValues['variable_9'] > $Valor_Ideal[9] && $consolidadoValues['variable_9'] <=$Valor_Maximo[9]){
							...
							if($consolidadoValues['variable_9'] >$Valor_Maximo[9]){				
								$Dato='Valor Maximo';
								$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de: ".$consolidadoValues['variable_9'].", Ya que el valor máximo permitido es de: ".$Valor_Maximo[9].".";
							}
		...
		if(!empty($consolidadoValues['variable_10'])){			
			...
			if($consolidadoValues['variable_10'] != $Valor_Ideal[10]){
				...
				if($consolidadoValues['variable_10'] <$Valor_Minimo[10]){
					$Dato='Valor Minimo';
					$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de:".$consolidadoValues['variable_10'].", Ya que el valor mínimo permitido es de: ".$Valor_Minimo[10].".";
				}else{
					if($consolidadoValues['variable_10'] < $Valor_Ideal[10] && $consolidadoValues['variable_10'] >=$Valor_Minimo[10]){				
						...
						if($consolidadoValues['variable_10'] > $Valor_Ideal[10] && $consolidadoValues['variable_10'] <=$Valor_Maximo[10]){
							...
							if($consolidadoValues['variable_10'] >$Valor_Maximo[10]){				
								$Dato='Valor Maximo';
								$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de: ".$consolidadoValues['variable_10'].", Ya que el valor máximo permitido es de: ".$Valor_Maximo[10].".";
							}
		...
		if(!empty($consolidadoValues['variable_11'])){			
			...
			if($consolidadoValues['variable_11'] != $Valor_Ideal[11]){
				...
				if($consolidadoValues['variable_11'] <$Valor_Minimo[11]){
					$Dato='Valor Minimo';
					$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de:".$consolidadoValues['variable_11'].", Ya que el valor mínimo permitido es de: ".$Valor_Minimo[11].".";
				}else{
					if($consolidadoValues['variable_11'] < $Valor_Ideal[11] && $consolidadoValues['variable_11'] >=$Valor_Minimo[11]){				
						...
						if($consolidadoValues['variable_11'] > $Valor_Ideal[11] && $consolidadoValues['variable_11'] <=$Valor_Maximo[11]){
							...
							if($consolidadoValues['variable_11'] >$Valor_Maximo[11]){				
								$Dato='Valor Maximo';
								$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de: ".$consolidadoValues['variable_11'].", Ya que el valor máximo permitido es de: ".$Valor_Maximo[11].".";
							}
		...
		if(!empty($consolidadoValues['variable_12'])){			
			...
			if($consolidadoValues['variable_12'] != $Valor_Ideal[12]){
				...
				if($consolidadoValues['variable_12'] <$Valor_Minimo[12]){
					$Dato='Valor Minimo';
					$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de:".$consolidadoValues['variable_12'].", Ya que el valor mínimo permitido es de: ".$Valor_Minimo[12].".";
				}else{
					if($consolidadoValues['variable_12'] < $Valor_Ideal[12] && $consolidadoValues['variable_12'] >=$Valor_Minimo[12]){				
						...
						if($consolidadoValues['variable_12'] > $Valor_Ideal[12] && $consolidadoValues['variable_12'] <=$Valor_Maximo[12]){
							...
							if($consolidadoValues['variable_12'] >$Valor_Maximo[12]){				
								$Dato='Valor Maximo';
								$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de: ".$consolidadoValues['variable_12'].", Ya que el valor máximo permitido es de: ".$Valor_Maximo[12].".";
							}
		...
		if(!empty($consolidadoValues['variable_13'])){			
			...
			if($consolidadoValues['variable_13'] != $Valor_Ideal[13]){
				...
				if($consolidadoValues['variable_13'] <$Valor_Minimo[13]){
					$Dato='Valor Minimo';
					$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de:".$consolidadoValues['variable_13'].", Ya que el valor mínimo permitido es de: ".$Valor_Minimo[13].".";
				}else{
					if($consolidadoValues['variable_13'] < $Valor_Ideal[13] && $consolidadoValues['variable_13'] >=$Valor_Minimo[13]){				
						...
						if($consolidadoValues['variable_13'] > $Valor_Ideal[13] && $consolidadoValues['variable_13'] <=$Valor_Maximo[13]){
							...
							if($consolidadoValues['variable_13'] >$Valor_Maximo[13]){				
								$Dato='Valor Maximo';
								$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de: ".$consolidadoValues['variable_13'].", Ya que el valor máximo permitido es de: ".$Valor_Maximo[13].".";
							}
		...
		if(!empty($consolidadoValues['variable_14'])){			
			...
			if($consolidadoValues['variable_14'] != $Valor_Ideal[14]){
				...
				if($consolidadoValues['variable_14'] <$Valor_Minimo[14]){
					$Dato='Valor Minimo';
					$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de:".$consolidadoValues['variable_14'].", Ya que el valor mínimo permitido es de: ".$Valor_Minimo[14].".";
				}else{
					if($consolidadoValues['variable_14'] < $Valor_Ideal[14] && $consolidadoValues['variable_14'] >=$Valor_Minimo[14]){				
						...
						if($consolidadoValues['variable_14'] > $Valor_Ideal[14] && $consolidadoValues['variable_14'] <=$Valor_Maximo[14]){
							...
							if($consolidadoValues['variable_14'] >$Valor_Maximo[14]){				
								$Dato='Valor Maximo';
								$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de: ".$consolidadoValues['variable_14'].", Ya que el valor máximo permitido es de: ".$Valor_Maximo[14].".";
							}
		...
		if(!empty($consolidadoValues['variable_15'])){			
			...
			if($consolidadoValues['variable_15'] != $Valor_Ideal[15]){
				...
				if($consolidadoValues['variable_15'] <$Valor_Minimo[15]){
					$Dato='Valor Minimo';
					$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de:".$consolidadoValues['variable_15'].", Ya que el valor mínimo permitido es de: ".$Valor_Minimo[15].".";
				}else{
					if($consolidadoValues['variable_15'] < $Valor_Ideal[15] && $consolidadoValues['variable_15'] >=$Valor_Minimo[15]){				
						...
						if($consolidadoValues['variable_15'] > $Valor_Ideal[15] && $consolidadoValues['variable_15'] <=$Valor_Maximo[15]){
							...
							if($consolidadoValues['variable_15'] >$Valor_Maximo[15]){				
								$Dato='Valor Maximo';
								$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de: ".$consolidadoValues['variable_15'].", Ya que el valor máximo permitido es de: ".$Valor_Maximo[15].".";
							}
		...
		if(!empty($consolidadoValues['variable_16'])){			
			...
			if($consolidadoValues['variable_16'] != $Valor_Ideal[16]){
				...
				if($consolidadoValues['variable_16'] <$Valor_Minimo[16]){
					$Dato='Valor Minimo';
					$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de:".$consolidadoValues['variable_16'].", Ya que el valor mínimo permitido es de: ".$Valor_Minimo[16].".";
				}else{
					if($consolidadoValues['variable_16'] < $Valor_Ideal[16] && $consolidadoValues['variable_16'] >=$Valor_Minimo[16]){				
						...
						if($consolidadoValues['variable_16'] > $Valor_Ideal[16] && $consolidadoValues['variable_16'] <=$Valor_Maximo[16]){
							...
							if($consolidadoValues['variable_16'] >$Valor_Maximo[16]){				
								$Dato='Valor Maximo';
								$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de: ".$consolidadoValues['variable_16'].", Ya que el valor máximo permitido es de: ".$Valor_Maximo[16].".";
							}
		...
		if(!empty($consolidadoValues['variable_17'])){			
			...
			if($consolidadoValues['variable_17'] != $Valor_Ideal[17]){
				...
				if($consolidadoValues['variable_17'] <$Valor_Minimo[17]){
					$Dato='Valor Minimo';
					$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de:".$consolidadoValues['variable_17'].", Ya que el valor mínimo permitido es de: ".$Valor_Minimo[17].".";
				}else{
					if($consolidadoValues['variable_17'] < $Valor_Ideal[17] && $consolidadoValues['variable_17'] >=$Valor_Minimo[17]){				
						...
						if($consolidadoValues['variable_17'] > $Valor_Ideal[17] && $consolidadoValues['variable_17'] <=$Valor_Maximo[17]){
							...
							if($consolidadoValues['variable_17'] >$Valor_Maximo[17]){				
								$Dato='Valor Maximo';
								$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de: ".$consolidadoValues['variable_17'].", Ya que el valor máximo permitido es de: ".$Valor_Maximo[17].".";
							}
		...
		if(!empty($consolidadoValues['variable_18'])){			
			...
			if($consolidadoValues['variable_18'] != $Valor_Ideal[18]){
				...
				if($consolidadoValues['variable_18'] <$Valor_Minimo[18]){
					$Dato='Valor Minimo';
					$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de:".$consolidadoValues['variable_18'].", Ya que el valor mínimo permitido es de: ".$Valor_Minimo[18].".";
				}else{
					if($consolidadoValues['variable_18'] < $Valor_Ideal[18] && $consolidadoValues['variable_18'] >=$Valor_Minimo[18]){				
						...
						if($consolidadoValues['variable_18'] > $Valor_Ideal[18] && $consolidadoValues['variable_18'] <=$Valor_Maximo[18]){
							...
							if($consolidadoValues['variable_18'] >$Valor_Maximo[18]){				
								$Dato='Valor Maximo';
								$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de: ".$consolidadoValues['variable_18'].", Ya que el valor máximo permitido es de: ".$Valor_Maximo[18].".";
							}
		...
		if(!empty($consolidadoValues['variable_19'])){			
			...
			if($consolidadoValues['variable_19'] != $Valor_Ideal[19]){
				...
				if($consolidadoValues['variable_19'] <$Valor_Minimo[19]){
					$Dato='Valor Minimo';
					$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de:".$consolidadoValues['variable_19'].", Ya que el valor mínimo permitido es de: ".$Valor_Minimo[19].".";
				}else{
					if($consolidadoValues['variable_19'] < $Valor_Ideal[19] && $consolidadoValues['variable_19'] >=$Valor_Minimo[19]){				
						...
						if($consolidadoValues['variable_19'] > $Valor_Ideal[19] && $consolidadoValues['variable_19'] <=$Valor_Maximo[19]){
							...
							if($consolidadoValues['variable_19'] >$Valor_Maximo[19]){				
								$Dato='Valor Maximo';
								$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de: ".$consolidadoValues['variable_19'].", Ya que el valor máximo permitido es de: ".$Valor_Maximo[19].".";
							}
		...
		if(!empty($consolidadoValues['variable_20'])){			
			...
			if($consolidadoValues['variable_20'] != $Valor_Ideal[20]){
				...
				if($consolidadoValues['variable_20'] <$Valor_Minimo[20]){
					$Dato='Valor Minimo';
					$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de:".$consolidadoValues['variable_20'].", Ya que el valor mínimo permitido es de: ".$Valor_Minimo[20].".";
				}else{
					if($consolidadoValues['variable_20'] < $Valor_Ideal[20] && $consolidadoValues['variable_20'] >=$Valor_Minimo[20]){				
						...
						if($consolidadoValues['variable_20'] > $Valor_Ideal[20] && $consolidadoValues['variable_20'] <=$Valor_Maximo[20]){
							...
							if($consolidadoValues['variable_20'] >$Valor_Maximo[20]){				
								$Dato='Valor Maximo';
								$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de: ".$consolidadoValues['variable_20'].", Ya que el valor máximo permitido es de: ".$Valor_Maximo[20].".";
							}
		...
		if(!empty($consolidadoValues['variable_21'])){			
			...
			if($consolidadoValues['variable_21'] != $Valor_Ideal[21]){
				...
				if($consolidadoValues['variable_21'] <$Valor_Minimo[21]){
					$Dato='Valor Minimo';
					$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de:".$consolidadoValues['variable_21'].", Ya que el valor mínimo permitido es de: ".$Valor_Minimo[21].".";
				}else{
					if($consolidadoValues['variable_21'] < $Valor_Ideal[21] && $consolidadoValues['variable_21'] >=$Valor_Minimo[21]){				
						...
						if($consolidadoValues['variable_21'] > $Valor_Ideal[21] && $consolidadoValues['variable_21'] <=$Valor_Maximo[21]){
							...
							if($consolidadoValues['variable_21'] >$Valor_Maximo[21]){				
								$Dato='Valor Maximo';
								$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de: ".$consolidadoValues['variable_21'].", Ya que el valor máximo permitido es de: ".$Valor_Maximo[21].".";
							}
		...
		if(!empty($consolidadoValues['variable_22'])){			
			...
			if($consolidadoValues['variable_22'] != $Valor_Ideal[22]){
				...
				if($consolidadoValues['variable_22'] <$Valor_Minimo[22]){
					$Dato='Valor Minimo';
					$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de:".$consolidadoValues['variable_22'].", Ya que el valor mínimo permitido es de: ".$Valor_Minimo[22].".";
				}else{
					if($consolidadoValues['variable_22'] < $Valor_Ideal[22] && $consolidadoValues['variable_22'] >=$Valor_Minimo[22]){				
						...
						if($consolidadoValues['variable_22'] > $Valor_Ideal[22] && $consolidadoValues['variable_22'] <=$Valor_Maximo[22]){
							...
							if($consolidadoValues['variable_22'] >$Valor_Maximo[22]){				
								$Dato='Valor Maximo';
								$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de: ".$consolidadoValues['variable_22'].", Ya que el valor máximo permitido es de: ".$Valor_Maximo[22].".";
							}
		...
		if(!empty($consolidadoValues['variable_23'])){			
			...
			if($consolidadoValues['variable_23'] != $Valor_Ideal[23]){
				...
				if($consolidadoValues['variable_23'] <$Valor_Minimo[23]){
					$Dato='Valor Minimo';
					$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de:".$consolidadoValues['variable_23'].", Ya que el valor mínimo permitido es de: ".$Valor_Minimo[23].".";
				}else{
					if($consolidadoValues['variable_23'] < $Valor_Ideal[23] && $consolidadoValues['variable_23'] >=$Valor_Minimo[23]){				
						...
						if($consolidadoValues['variable_23'] > $Valor_Ideal[23] && $consolidadoValues['variable_23'] <=$Valor_Maximo[23]){
							...
							if($consolidadoValues['variable_23'] >$Valor_Maximo[23]){				
								$Dato='Valor Maximo';
								$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de: ".$consolidadoValues['variable_23'].", Ya que el valor máximo permitido es de: ".$Valor_Maximo[23].".";
							}
		...
		if(!empty($consolidadoValues['variable_24'])){			
			...
			if($consolidadoValues['variable_24'] != $Valor_Ideal[24]){
				...
				if($consolidadoValues['variable_24'] <$Valor_Minimo[24]){
					$Dato='Valor Minimo';
					$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de:".$consolidadoValues['variable_24'].", Ya que el valor mínimo permitido es de: ".$Valor_Minimo[24].".";
				}else{
					if($consolidadoValues['variable_24'] < $Valor_Ideal[24] && $consolidadoValues['variable_24'] >=$Valor_Minimo[24]){				
						...
						if($consolidadoValues['variable_24'] > $Valor_Ideal[24] && $consolidadoValues['variable_24'] <=$Valor_Maximo[24]){
							...
							if($consolidadoValues['variable_24'] >$Valor_Maximo[24]){				
								$Dato='Valor Maximo';
								$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de: ".$consolidadoValues['variable_24'].", Ya que el valor máximo permitido es de: ".$Valor_Maximo[24].".";
							}
		...
		if(!empty($consolidadoValues['variable_25'])){			
			...
			if($consolidadoValues['variable_25'] != $Valor_Ideal[25]){
				...
				if($consolidadoValues['variable_25'] <$Valor_Minimo[25]){
					$Dato='Valor Minimo';
					$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de:".$consolidadoValues['variable_25'].", Ya que el valor mínimo permitido es de: ".$Valor_Minimo[25].".";
				}else{
					if($consolidadoValues['variable_25'] < $Valor_Ideal[25] && $consolidadoValues['variable_25'] >=$Valor_Minimo[25]){				
						...
						if($consolidadoValues['variable_25'] > $Valor_Ideal[25] && $consolidadoValues['variable_25'] <=$Valor_Maximo[25]){
							...
							if($consolidadoValues['variable_25'] >$Valor_Maximo[25]){				
								$Dato='Valor Maximo';
								$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de: ".$consolidadoValues['variable_25'].", Ya que el valor máximo permitido es de: ".$Valor_Maximo[25].".";
							}
		...
		if(!empty($consolidadoValues['variable_26'])){			
			...
			if($consolidadoValues['variable_26'] != $Valor_Ideal[26]){
				...
				if($consolidadoValues['variable_26'] <$Valor_Minimo[26]){
					$Dato='Valor Minimo';
					$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de:".$consolidadoValues['variable_26'].", Ya que el valor mínimo permitido es de: ".$Valor_Minimo[26].".";
				}else{
					if($consolidadoValues['variable_26'] < $Valor_Ideal[26] && $consolidadoValues['variable_26'] >=$Valor_Minimo[26]){				
						...
						if($consolidadoValues['variable_26'] > $Valor_Ideal[26] && $consolidadoValues['variable_26'] <=$Valor_Maximo[26]){
							...
							if($consolidadoValues['variable_26'] >$Valor_Maximo[26]){				
								$Dato='Valor Maximo';
								$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de: ".$consolidadoValues['variable_26'].", Ya que el valor máximo permitido es de: ".$Valor_Maximo[26].".";
							}
		...
		if(!empty($consolidadoValues['variable_27'])){			
			...
			if($consolidadoValues['variable_27'] != $Valor_Ideal[27]){
				...
				if($consolidadoValues['variable_27'] <$Valor_Minimo[27]){
					$Dato='Valor Minimo';
					$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de:".$consolidadoValues['variable_27'].", Ya que el valor mínimo permitido es de: ".$Valor_Minimo[27].".";
				}else{
					if($consolidadoValues['variable_27'] < $Valor_Ideal[27] && $consolidadoValues['variable_27'] >=$Valor_Minimo[27]){				
						...
						if($consolidadoValues['variable_27'] > $Valor_Ideal[27] && $consolidadoValues['variable_27'] <=$Valor_Maximo[27]){
							...
							if($consolidadoValues['variable_27'] >$Valor_Maximo[27]){				
								$Dato='Valor Maximo';
								$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de: ".$consolidadoValues['variable_27'].", Ya que el valor máximo permitido es de: ".$Valor_Maximo[27].".";
							}
		...
		if(!empty($consolidadoValues['variable_28'])){			
			...
			if($consolidadoValues['variable_28'] != $Valor_Ideal[28]){
				...
				if($consolidadoValues['variable_28'] <$Valor_Minimo[28]){
					$Dato='Valor Minimo';
					$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de:".$consolidadoValues['variable_28'].", Ya que el valor mínimo permitido es de: ".$Valor_Minimo[28].".";
				}else{
					if($consolidadoValues['variable_28'] < $Valor_Ideal[28] && $consolidadoValues['variable_28'] >=$Valor_Minimo[28]){				
						...
						if($consolidadoValues['variable_28'] > $Valor_Ideal[28] && $consolidadoValues['variable_28'] <=$Valor_Maximo[28]){
							...
							if($consolidadoValues['variable_28'] >$Valor_Maximo[28]){				
								$Dato='Valor Maximo';
								$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de: ".$consolidadoValues['variable_28'].", Ya que el valor máximo permitido es de: ".$Valor_Maximo[28].".";
							}
		...
		if(!empty($consolidadoValues['variable_29'])){			
			...
			if($consolidadoValues['variable_29'] != $Valor_Ideal[29]){
				...
				if($consolidadoValues['variable_29'] <$Valor_Minimo[29]){
					$Dato='Valor Minimo';
					$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de:".$consolidadoValues['variable_29'].", Ya que el valor mínimo permitido es de: ".$Valor_Minimo[29].".";
				}else{
					if($consolidadoValues['variable_29'] < $Valor_Ideal[29] && $consolidadoValues['variable_29'] >=$Valor_Minimo[29]){				
						...
						if($consolidadoValues['variable_29'] > $Valor_Ideal[29] && $consolidadoValues['variable_29'] <=$Valor_Maximo[29]){
							...
							if($consolidadoValues['variable_29'] >$Valor_Maximo[29]){				
								$Dato='Valor Maximo';
								$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de: ".$consolidadoValues['variable_29'].", Ya que el valor máximo permitido es de: ".$Valor_Maximo[29].".";
							}
		...
		if(!empty($consolidadoValues['variable_30'])){			
			...
			if($consolidadoValues['variable_30'] != $Valor_Ideal[30]){
				...
				if($consolidadoValues['variable_30'] <$Valor_Minimo[30]){
					$Dato='Valor Minimo';
					$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de:".$consolidadoValues['variable_30'].", Ya que el valor mínimo permitido es de: ".$Valor_Minimo[30].".";
				}else{
					if($consolidadoValues['variable_30'] < $Valor_Ideal[30] && $consolidadoValues['variable_30'] >=$Valor_Minimo[30]){				
						...
						if($consolidadoValues['variable_30'] > $Valor_Ideal[30] && $consolidadoValues['variable_30'] <=$Valor_Maximo[30]){
							...
							if($consolidadoValues['variable_30'] >$Valor_Maximo[30]){				
								$Dato='Valor Maximo';
								$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de: ".$consolidadoValues['variable_30'].", Ya que el valor máximo permitido es de: ".$Valor_Maximo[30].".";
							}
		...
		if(!empty($consolidadoValues['variable_31'])){			
			...
			if($consolidadoValues['variable_31'] != $Valor_Ideal[31]){
				...
				if($consolidadoValues['variable_31'] <$Valor_Minimo[31]){
					$Dato='Valor Minimo';
					$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de:".$consolidadoValues['variable_31'].", Ya que el valor mínimo permitido es de: ".$Valor_Minimo[31].".";
				}else{
					if($consolidadoValues['variable_31'] < $Valor_Ideal[31] && $consolidadoValues['variable_31'] >=$Valor_Minimo[31]){				
						...
						if($consolidadoValues['variable_31'] > $Valor_Ideal[31] && $consolidadoValues['variable_31'] <=$Valor_Maximo[31]){
							...
							if($consolidadoValues['variable_31'] >$Valor_Maximo[31]){				
								$Dato='Valor Maximo';
								$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de: ".$consolidadoValues['variable_31'].", Ya que el valor máximo permitido es de: ".$Valor_Maximo[31].".";
							}
		...
		if(!empty($consolidadoValues['variable_32'])){			
			...
			if($consolidadoValues['variable_32'] != $Valor_Ideal[32]){
				...
				if($consolidadoValues['variable_32'] <$Valor_Minimo[32]){
					$Dato='Valor Minimo';
					$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de:".$consolidadoValues['variable_32'].", Ya que el valor mínimo permitido es de: ".$Valor_Minimo[32].".";
				}else{
					if($consolidadoValues['variable_32'] < $Valor_Ideal[32] && $consolidadoValues['variable_32'] >=$Valor_Minimo[32]){				
						...
						if($consolidadoValues['variable_32'] > $Valor_Ideal[32] && $consolidadoValues['variable_32'] <=$Valor_Maximo[32]){
							...
							if($consolidadoValues['variable_32'] >$Valor_Maximo[32]){				
								$Dato='Valor Maximo';
								$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de: ".$consolidadoValues['variable_32'].", Ya que el valor máximo permitido es de: ".$Valor_Maximo[32].".";
							}
		...
		if(!empty($consolidadoValues['variable_33'])){			
			...
			if($consolidadoValues['variable_33'] != $Valor_Ideal[33]){
				...
				if($consolidadoValues['variable_33'] <$Valor_Minimo[33]){
					$Dato='Valor Minimo';
					$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de:".$consolidadoValues['variable_33'].", Ya que el valor mínimo permitido es de: ".$Valor_Minimo[33].".";
				}else{
					if($consolidadoValues['variable_33'] < $Valor_Ideal[33] && $consolidadoValues['variable_33'] >=$Valor_Minimo[33]){				
						...
						if($consolidadoValues['variable_33'] > $Valor_Ideal[33] && $consolidadoValues['variable_33'] <=$Valor_Maximo[33]){
							...
							if($consolidadoValues['variable_33'] >$Valor_Maximo[33]){				
								$Dato='Valor Maximo';
								$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de: ".$consolidadoValues['variable_33'].", Ya que el valor máximo permitido es de: ".$Valor_Maximo[33].".";
							}
		...
		if(!empty($consolidadoValues['variable_34'])){			
			...
			if($consolidadoValues['variable_34'] != $Valor_Ideal[34]){
				...
				if($consolidadoValues['variable_34'] <$Valor_Minimo[34]){
					$Dato='Valor Minimo';
					$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de:".$consolidadoValues['variable_34'].", Ya que el valor mínimo permitido es de: ".$Valor_Minimo[34].".";
				}else{
					if($consolidadoValues['variable_34'] < $Valor_Ideal[34] && $consolidadoValues['variable_34'] >=$Valor_Minimo[34]){				
						...
						if($consolidadoValues['variable_34'] > $Valor_Ideal[34] && $consolidadoValues['variable_34'] <=$Valor_Maximo[34]){
							...
							if($consolidadoValues['variable_34'] >$Valor_Maximo[34]){				
								$Dato='Valor Maximo';
								$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de: ".$consolidadoValues['variable_34'].", Ya que el valor máximo permitido es de: ".$Valor_Maximo[34].".";
							}
		...
		if(!empty($consolidadoValues['variable_35'])){			
			...
			if($consolidadoValues['variable_35'] != $Valor_Ideal[35]){
				...
				if($consolidadoValues['variable_35'] <$Valor_Minimo[35]){
					$Dato='Valor Minimo';
					$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de:".$consolidadoValues['variable_35'].", Ya que el valor mínimo permitido es de: ".$Valor_Minimo[35].".";
				}else{
					if($consolidadoValues['variable_35'] < $Valor_Ideal[35] && $consolidadoValues['variable_35'] >=$Valor_Minimo[35]){				
						...
						if($consolidadoValues['variable_35'] > $Valor_Ideal[35] && $consolidadoValues['variable_35'] <=$Valor_Maximo[35]){
							...
							if($consolidadoValues['variable_35'] >$Valor_Maximo[35]){				
								$Dato='Valor Maximo';
								$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de: ".$consolidadoValues['variable_35'].", Ya que el valor máximo permitido es de: ".$Valor_Maximo[35].".";
							}
		...
		if(!empty($consolidadoValues['variable_36'])){			
			...
			if($consolidadoValues['variable_36'] != $Valor_Ideal[36]){
				...
				if($consolidadoValues['variable_36'] <$Valor_Minimo[36]){
					$Dato='Valor Minimo';
					$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de:".$consolidadoValues['variable_36'].", Ya que el valor mínimo permitido es de: ".$Valor_Minimo[36].".";
				}else{
					if($consolidadoValues['variable_36'] < $Valor_Ideal[36] && $consolidadoValues['variable_36'] >=$Valor_Minimo[36]){				
						...
						if($consolidadoValues['variable_36'] > $Valor_Ideal[36] && $consolidadoValues['variable_36'] <=$Valor_Maximo[36]){
							...
							if($consolidadoValues['variable_36'] >$Valor_Maximo[36]){				
								$Dato='Valor Maximo';
								$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de: ".$consolidadoValues['variable_36'].", Ya que el valor máximo permitido es de: ".$Valor_Maximo[36].".";
							}
		...
		if(!empty($consolidadoValues['variable_37'])){			
			...
			if($consolidadoValues['variable_37'] != $Valor_Ideal[37]){
				...
				if($consolidadoValues['variable_37'] <$Valor_Minimo[37]){
					$Dato='Valor Minimo';
					$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de:".$consolidadoValues['variable_37'].", Ya que el valor mínimo permitido es de: ".$Valor_Minimo[37].".";
				}else{
					if($consolidadoValues['variable_37'] < $Valor_Ideal[37] && $consolidadoValues['variable_37'] >=$Valor_Minimo[37]){				
						...
						if($consolidadoValues['variable_37'] > $Valor_Ideal[37] && $consolidadoValues['variable_37'] <=$Valor_Maximo[37]){
							...
							if($consolidadoValues['variable_37'] >$Valor_Maximo[37]){				
								$Dato='Valor Maximo';
								$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de: ".$consolidadoValues['variable_37'].", Ya que el valor máximo permitido es de: ".$Valor_Maximo[37].".";
							}
		...
		if(!empty($consolidadoValues['variable_38'])){			
			...
			if($consolidadoValues['variable_38'] != $Valor_Ideal[38]){
				...
				if($consolidadoValues['variable_38'] <$Valor_Minimo[38]){
					$Dato='Valor Minimo';
					$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de:".$consolidadoValues['variable_38'].", Ya que el valor mínimo permitido es de: ".$Valor_Minimo[38].".";
				}else{
					if($consolidadoValues['variable_38'] < $Valor_Ideal[38] && $consolidadoValues['variable_38'] >=$Valor_Minimo[38]){				
						...
						if($consolidadoValues['variable_38'] > $Valor_Ideal[38] && $consolidadoValues['variable_38'] <=$Valor_Maximo[38]){
							...
							if($consolidadoValues['variable_38'] >$Valor_Maximo[38]){				
								$Dato='Valor Maximo';
								$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de: ".$consolidadoValues['variable_38'].", Ya que el valor máximo permitido es de: ".$Valor_Maximo[38].".";
							}
		...
		if(!empty($consolidadoValues['variable_39'])){			
			...
			if($consolidadoValues['variable_39'] != $Valor_Ideal[39]){
				...
				if($consolidadoValues['variable_39'] <$Valor_Minimo[39]){
					$Dato='Valor Minimo';
					$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de:".$consolidadoValues['variable_39'].", Ya que el valor mínimo permitido es de: ".$Valor_Minimo[39].".";
				}else{
					if($consolidadoValues['variable_39'] < $Valor_Ideal[39] && $consolidadoValues['variable_39'] >=$Valor_Minimo[39]){				
						...
						if($consolidadoValues['variable_39'] > $Valor_Ideal[39] && $consolidadoValues['variable_39'] <=$Valor_Maximo[39]){
							...
							if($consolidadoValues['variable_39'] >$Valor_Maximo[39]){				
								$Dato='Valor Maximo';
								$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de: ".$consolidadoValues['variable_39'].", Ya que el valor máximo permitido es de: ".$Valor_Maximo[39].".";
							}
		...
		if(!empty($consolidadoValues['variable_40'])){			
			...
			if($consolidadoValues['variable_40'] != $Valor_Ideal[40]){
				...
				if($consolidadoValues['variable_40'] <$Valor_Minimo[40]){
					$Dato='Valor Minimo';
					$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de:".$consolidadoValues['variable_40'].", Ya que el valor mínimo permitido es de: ".$Valor_Minimo[40].".";
				}else{
					if($consolidadoValues['variable_40'] < $Valor_Ideal[40] && $consolidadoValues['variable_40'] >=$Valor_Minimo[40]){				
						...
						if($consolidadoValues['variable_40'] > $Valor_Ideal[40] && $consolidadoValues['variable_40'] <=$Valor_Maximo[40]){
							...
							if($consolidadoValues['variable_40'] >$Valor_Maximo[40]){				
								$Dato='Valor Maximo';
								$Alerta="¡¡ ALERTA !! Si estás viendo este mensaje es porque el sistema presento un valor de: ".$consolidadoValues['variable_40'].", Ya que el valor máximo permitido es de: ".$Valor_Maximo[40].".";
							}
