				...
				<table class="table table-user-information">
					<div class="row">						
						<tbody>	
							...
							<tr>
								<td>								
									<b><strong><font size ="2", color color="#000000" face="Tahoma">Selecciona la RUTA:</font></strong></b>
								</td>
								<td>
...
<script type="text/javascript">
	...
	$('.NuevaAsignacion').click(function(){
		LimpiarFormulario();
		RemoverDataCombobox(document.getElementById("id_nombre_usuario"));	

		Cargar_Usuarios();
		$('#Modal_Registro_Asignacion').modal('show');
	});
	$('.BuscarAsignacion').click(function(){
		RemoverDataCombobox(document.getElementById("NombreUsuarioBuscar"));
		Cargar_Usuarios();
		LimpiarEnBuscar();
	...
	$('body').delegate('.Editar_Asignacion','click',function(){
		// RemoverDataCombobox(document.getElementById("id_nombre_usuario_editar"));
		// Cargar_Usuarios();	
			...
			$.ajax({
				...
				success:function(respuesta){
					if(respuesta==0){ 
						...
						$("#estilo_mensaje2").fadeTo(5500, 500).slideUp(500, function(){
							$("#estilo_mensaje2").hide();
						});
					}else{
        		...
        		success:function(respuesta){
        			...
        			if(respuesta.error==false){
        				...
        				$.each(respuesta.errors,function(index, error){ 
        					$('#id_validacion').html('<center><i class="fa fa-times fa-2x" aria-hidden="true"></i>'+error+'</center>');
        					$("#id_validacion").css("fontSize", 15);				
        					$("#id_validacion").css("font-weight","Bold"); 
        					$("#id_validacion").show();
        					$("#estilo_mensaje").show();
        					$("#estilo_mensaje").fadeTo(7500, 500).slideUp(500, function(){
        						$("#estilo_mensaje").hide();
        					});
        				}); 
        			}
        			if(respuesta==3){
        				$('#ModalConfirmar').modal('hide');
        				$('#id_validacion').html('<center><i class="fa fa-times fa-2x" aria-hidden="true"></i><strong>Error: Se encontró una asignación ingresada con el mismo turno y formulario.</strong></center>');
        				$("#id_validacion").css("fontSize", 15);				
        				$("#id_validacion").css("font-weight","Bold"); 
        				$("#id_validacion").show();
        				$("#estilo_mensaje").show();
        			...
        			if(respuesta==4){
        				$('#ModalConfirmar').modal('hide');
        				$('#id_validacion').html('');
        				$('#id_validacion').html('<center><i class="fa fa-times fa-2x" aria-hidden="true"></i><strong> Error: La RUTA seleccionada no tiene ningún parámetro asociado.</strong></center>');
        				$("#id_validacion").css("fontSize", 15);				
        				$("#id_validacion").css("font-weight","Bold"); 
        				$("#id_validacion").show();
        				$("#estilo_mensaje").show();
